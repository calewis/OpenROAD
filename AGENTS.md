...
2. Run `clang-format -i <files>` for C++ files before commit. **Prefer `clang-format-18`** to match upstream CI if it is available. **NEVER** run it for `src/sta/*` and `*.i` files.
